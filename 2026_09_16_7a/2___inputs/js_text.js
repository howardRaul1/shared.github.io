...
function applyBookmarkedSettings() {
    ...
    document.querySelectorAll(".lvB1").forEach(function (lvB1) {
        const wordId = lvB1.previousElementSibling?.dataset.idInJwordlist;
        const isBookmarked = settings.includes(Number(wordId));

        lvB1.classList.toggle("bookmarked", isBookmarked);
    });
}
...
function saveBookmarkedSettings() {
    ...
    document.querySelectorAll(".lvB1.bookmarked").forEach(function (lvB1) {
        const wordId = lvB1.previousElementSibling?.dataset.idInJwordlist;
...
function scrollToLastBookmarked() {
    const bookmarked = document.querySelectorAll(".lvB1.bookmarked");

    if (bookmarked.length === 0) {
        return;
    }

    const lastBookmarked = bookmarked[bookmarked.length - 1];

    const rect = lastBookmarked.getBoundingClientRect();
    const targetY = window.scrollY + rect.top - (window.innerHeight * BOOKMARK_DIV_PERCENTAGE_H_FROM_TOP);   // position element 20% down from the top of the viewport
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    document.querySelectorAll(".lvB1").forEach(function (lvB1) {
        lvB1.addEventListener("click", function () {
    ...
    const phraseEndingPunctuationLv3 =
        document.querySelectorAll(".lv3.phrase-ending-punctuation");
    ...
    phraseEndingPunctuationLv3.forEach(function (punctuation) {
        punctuation.addEventListener("click", async function () {
            ...
            // Generate the hash-based MP3 filename.
            const hash = MY_HASH_1(text);
            const fileName = hash + ".mp3";

            // Save the relative path in JSON.
            lv1Sentences[text] =
                "../../7___html_settings/saved_phrase_mp3/" + fileName;

            await saveLv1Sentences();

            console.log("Lv1 sentence mapping:", lv1Sentences);
            console.log("Sentence:", text);
            console.log("Audio path:", lv1Sentences[text]);
        });
    });
    ...
    const TL_ClearBookmarksButton = document.getElementById(
        "TL_ClearBookmarksButton"
    );

    TL_ClearBookmarksButton.addEventListener("click", function () {
        // 1. Remove the "bookmarked" class from every .lvB1.
        document.querySelectorAll(".lvB1.bookmarked").forEach(function (lvB1) {
            lvB1.classList.remove("bookmarked");
        });

        // 2. Clear saved bookmark settings.
        localStorage.removeItem(BOOKMARKED_SETTINGS_KEY);
    });
